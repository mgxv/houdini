...
# Single source of truth for the framework plist version and the
# `houdini version` subcommand. Bump this file in lockstep with the
# Homebrew formula's `url` tag on release.
[ -f VERSION ] || { echo "VERSION file missing" >&2; exit 1; }
VERSION="$(tr -d '[:space:]' < VERSION)"
[ -n "$VERSION" ] || { echo "VERSION file is empty" >&2; exit 1; }

# Regenerate Sources/Version.swift from VERSION so the binary knows
# what it is. Gitignored — the authoritative value lives in VERSION.
cat > Sources/Version.swift <<SWIFT
// Auto-generated by build.sh from VERSION. Do not edit.
